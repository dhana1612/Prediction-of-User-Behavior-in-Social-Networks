...
USERS = {
    "admin": {"password": "Chandra123", "email": "chandra@example.com", "address": "123 Main St"},
    "user1": {"password": "Chandra123", "email": "chandra@example.com", "address": "123 Main St"}
}
...
#Document Analysis Model home page
...
#After the user has uploaded the document output Will Display
@app.route('/analyze', methods=['POST'])

#Logic for Document Analysis Model
def analyze():
    ...
    # Assuming df is your DataFrame and 'Date' is the column to be parsed
    ...
    sns.countplot(x='day_of_week', data=df, palette='Set2', ax=ax, order=['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'])

    # Find the most active day and highlight it
    most_active_day = df['day_of_week'].mode().values[0]

    # Iterate through the patches (bars) and set the color for the most active day
    for patch in ax.patches:
        if patch.get_x() == ax.patches[df['day_of_week'].value_counts().index.get_loc(most_active_day)].get_x():
    ...
    fig.savefig('static/images/messages_by_day.png', dpi=100, bbox_inches='tight')
    ...
    return render_template('result.html',
                                    ...
                                    image_file='static/images/messages_by_day.png',
...
#Logic end for Document Analysis Model
...
#Paragraph Analysis Model Start
...
def paragraph_sentiment():
    ...
    label, translated_paragraph = asyncio.run(get_paragraph_sentiment(paragraph))
...
async def get_paragraph_sentiment(paragraph):
    # Translate the paragraph to English
    translator = Translator()
    translated = await translator.translate(paragraph, dest='en')
    translated_paragraph = translated.text
...
Behaviour = pipeline('sentiment-analysis')
